...
do_custom_action (Brightside *brightside, gint corner, gint action)
{
	...
	if (is_enabled || brightside->settings.corners[corner].custom_kill ||
			(brightside->settings.corners[corner].custom_out &&
			 strlen (brightside->settings.corners[corner].custom_out
				 ) != 0))
		show_dialog_timed_bar (brightside, ICON_CUSTOM, is_enabled);
	else
